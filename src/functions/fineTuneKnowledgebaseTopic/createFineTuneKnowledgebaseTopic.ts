...
> = async (event) => {
  try {
    const { name, description, meta_data, value, fine_tuned_model_id } =
      event.body;
    ...
    console.log(
      "🚀 ~ file: createFineTuneKnowledgebaseTopic.ts:75 ~ >= ~ knowledgebase:",
      knowledgebase
    );
    ...
    console.log(
      "🚀 ~ file: createFineTuneKnowledgebaseTopic.ts:92 ~ >= ~ params:",
      params
    );
    ...
    //   .catch((err) => {
    //     console.log(
    //       "🚀 ~ file: createFineTuneKnowledgebaseTopic.ts:101 ~ >= ~ err:"
    //     );
    //     console.log(err);
    //   });
