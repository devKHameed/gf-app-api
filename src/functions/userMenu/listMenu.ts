...
const lambdaHandler: ValidatedEventAPIGatewayProxyEvent = async (event) => {
  ...
  const template_id = event.queryStringParameters?.template_id;

  try {
    const connectionKnex = await connectKnex(databaseName);

    const menus = await connectionKnex<UserMenuItem>(USER_MENU_ITEM).where(
      "parent_menu",
      template_id
    );
    ...
    return {
      statusCode: 200,
      body: { message: "List of menus", data: menus },
    };
  } catch (error: unknown) {
