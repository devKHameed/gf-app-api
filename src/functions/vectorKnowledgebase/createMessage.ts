...
import { AccountUser, VectorKnowledgebase } from "types";
import { v4 } from "uuid";
import {
  ACCOUNT_VECTOR_KNOWLEDGEBASE_MESSAGES_TABLE_NAME,
...
const tableName = envTableNames.DYNAMODB_ACCT_VECTOR_KNOWLEDGEBASE_MESSAGES;
...
> = async (event) => {
  ...
  try {
    ...
    const params: DynamoDB.DocumentClient.PutItemInput = {
      TableName: tableName,
      Item: {
        id: `${accountId}:${slug}:${ACCOUNT_VECTOR_KNOWLEDGEBASE_MESSAGES_TABLE_NAME}`,
        slug: `false:${v4()}`,
        data: message,
        sent_by: "user",
        user_data: {
